...
export default function Sidebar({ onClose }: { collapsed?: boolean; onClose?: () => void }) {
  ...
  return (
    ...
    ">
      ...
      <div className="flex items-center gap-[10px] px-4 py-[14px] border-b border-[#1f1f23]
                      lg:px-4 md:justify-center md:px-0 lg:justify-start">
        <div
          className="w-8 h-8 rounded-sm flex items-center justify-center
                     font-display text-[13px] font-bold flex-shrink-0 border"
          style={{
        ...
        {/* Text hidden at tablet (md), shown at desktop (lg) and on mobile overlay */}
      ...
      <nav className="flex-1 flex flex-col gap-[2px] px-2 py-3 overflow-y-auto">
        ...
        <span className="text-[9px] font-semibold uppercase tracking-[0.1em] text-text-muted
                         px-2 pt-2 pb-1 md:hidden lg:block">
          Create
        </span>
        {CREATE_NAV.map(item => <NavItem key={item.to} {...item} />)}

        <span className="text-[9px] font-semibold uppercase tracking-[0.1em] text-text-muted
                         px-2 pt-3 pb-1 md:hidden lg:block">
          Plan
        </span>
        {PLAN_NAV.map(item => <NavItem key={item.to} {...item} />)}
      ...
      <div className="flex flex-col gap-[2px] px-2 py-3 border-t border-[#1f1f23]">
        ...
        >
          ...
          <span className="md:hidden lg:inline">Switch Persona</span>
        </button>

        <button className="flex items-center gap-[9px] px-[10px] py-2 rounded-sm text-[13px]
                           text-text-secondary hover:bg-bg-hover hover:text-text-primary
                           transition-colors duration-150 w-full
                           md:justify-center lg:justify-start">
          <span className="w-4 h-4 flex-shrink-0 opacity-70">
